...
resource "aws_ssm_parameter" "shopify_admin_api_domain" {
  name      = "/greatupsells/${terraform.workspace}/shopify-admin-api/domain"
  type      = "String"
  value     = var.shopify_admin_api_domain
  overwrite = true
  provider  = aws.region
}

resource "aws_ssm_parameter" "shopify_admin_api_url" {
  name      = "/greatupsells/${terraform.workspace}/shopify-admin-api/url"
  type      = "String"
...
resource "aws_route53_health_check" "shopify_admin_api" {
  fqdn              = aws_ssm_parameter.shopify_admin_api_domain.value
  port              = 443
  type              = "HTTPS"
  ...
  tags = {
    Name = "shopify-admin-api-${terraform.workspace}"
  }
}

resource "aws_cloudwatch_metric_alarm" "shopify_admin_api" {
  alarm_name          = "shopify-admin-api-alarm-${terraform.workspace}"
  ...
  dimensions = {
    HealthCheckId = aws_route53_health_check.shopify_admin_api.id
  }
}
...
resource "aws_ssm_parameter" "shopify_admin_api_health_check_id" {
  name      = "/greatupsells/${terraform.workspace}/shopify-admin-api/health-check-id"
  type      = "String"
  value     = aws_route53_health_check.shopify_admin_api.id
  overwrite = true
  provider  = aws.region
}
