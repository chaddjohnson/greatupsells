...
const processor = async (metadata, payload) => {
  try {
    ...
    await logger.info(`Theme "${shopifyThemeData.name}" (${shopifyThemeData.id}) published for shop ${shop.domain}`, {
      metadata,
      payload
    });

    await httpClient.post(`/shops/${shop._id}/theme-compatibility`);
    await httpClient.post(`/shops/${shop._id}/themes/${shopifyThemeId}/app-embed-block-install`);
  } catch (error) {
