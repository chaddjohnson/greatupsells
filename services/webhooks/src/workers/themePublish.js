  ...
  try {
    ...
    if (errors) {
      return await logger.error(
        `Error handling theme publish webhook`,
        errors,
        record
      );
    }
    ...
    await logger.debug(
      `Updating shop ${shop.domain} theme via webhook`,
      record
    );

    await httpClient.post(`/shops/${shop._id}/shopify-theme-update`);
  } catch (error) {
    await logger.error(`Error updating shop theme via webhook`, error, record);
  }
};
