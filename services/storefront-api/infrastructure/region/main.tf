...
resource "aws_cloudfront_distribution" "storefront_api" {
  enabled     = true
  aliases     = [var.storefront_api_domain]
  price_class = "PriceClass_All"

  origin {
    domain_name = aws_ssm_parameter.storefront_api_regional_domain.value
    origin_id   = "api"
...
resource "aws_route53_record" "storefront_api" {
  ...
  alias {
    name                   = aws_cloudfront_distribution.storefront_api.domain_name
    zone_id                = aws_cloudfront_distribution.storefront_api.hosted_zone_id
