...
resource "aws_cloudwatch_metric_alarm" "storefront_api" {
  alarm_name          = "storefront-api-alarm-${terraform.workspace}"
  ...
  dimensions = {
    HealthCheckId = aws_route53_health_check.storefront_api.id
  }
}
