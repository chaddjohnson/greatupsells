resource "aws_sqs_queue" "shop_order_import_dlq" {
  name                      = "shop-order-import-dlq-${terraform.workspace}"
...
resource "aws_sqs_queue_policy" "shop_order_import_policy" {
  queue_url = aws_sqs_queue.shop_order_import.id
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
        ...
        "Resource" : "${aws_sqs_queue.shop_order_import.arn}"
      }
    ]
  })
  provider = aws.region
...
resource "aws_ssm_parameter" "shop_order_import_queue_arn" {
  name      = "/greatupsells/${terraform.workspace}/queues/shop-order-import/arn"
  type      = "String"
  value     = aws_sqs_queue.shop_order_import.arn
  overwrite = true
  provider  = aws.region
}

resource "aws_ssm_parameter" "shop_order_import_queue_url" {
  name      = "/greatupsells/${terraform.workspace}/queues/shop-order-import/url"
  type      = "String"
  value     = aws_sqs_queue.shop_order_import.url
  overwrite = true
  provider  = aws.region
}
