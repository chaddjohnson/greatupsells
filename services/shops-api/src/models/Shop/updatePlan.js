...
const updatePlan = async (shop) => {
  if (!shop.plan.chargeId) {
    throw new Error(`Cannot update plan for shop as no recurring charge exists (${shop.toString()})`);
  }
  ...
  const chargeIsCanceled = recurringChargeData?.cancelled_on || recurringChargeData?.status === 'cancelled';

  const billingOn = recurringChargeData.billing_on && new Date(`${recurringChargeData.billing_on}T12:00:00Z`);
  ...
  const planIsInactive =
    !recurringChargeData || chargeIsCanceled || ['declined', 'expired'].includes(recurringChargeData.status);
  const resetPlan = !billingCycleActive && planIsInactive;

  // The plan should deactivated if the charge is frozen.
  const deactivatePlan = recurringChargeData?.status === 'frozen' && shop.plan.active;

  // The plan should be reactivated if the charge is active and the plan is inactive.
  const reactivatePlan = recurringChargeData?.status === 'active' && !shop.plan.active;
  ...
  if (resetPlan) {
    await logger.warn(`Resetting plan for shop as recurring charge is declined or expired (${shop.toString()})`);

    await shop.resetPlan();
  }

  if (deactivatePlan) {
    await logger.warn(`Deactivating plan for shop due to non-payment (${shop.toString()})`);
