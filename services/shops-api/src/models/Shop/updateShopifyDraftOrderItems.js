...
const updateOfferHits = async (draftOrder, discountedDraftOrderItem, quantityToRemove, attempts = 0) => {
  if (attempts >= 3) {
    return;
  }
  ...
  const offerHit = offerHits.find(
    ({ acceptedProducts }) =>
      !!acceptedProducts.find(({ shopifyVariantId }) => shopifyVariantId === discountedDraftOrderItem.variant_id)
  );
  ...
  const acceptedProduct = offerHit.acceptedProducts.find(
    ({ shopifyVariantId }) => shopifyVariantId === discountedDraftOrderItem.variant_id
  );
  ...
  offerHit.acceptedProducts = offerHit.acceptedProducts.filter(({ quantity }) => quantity > 0);
  ...
  if (remainingQuantityToRemove > 0) {
    await updateOfferHits(draftOrder, discountedDraftOrderItem, remainingQuantityToRemove, ++attempts);
  }
};

const updateItemQuantity = async (draftOrder, shopifyCartItems, shopifyCartItem) => {
  ...
  if (totalDraftOrderQuantity < totalShopifyCartQuantity) {
    if (nonDiscountedDraftOrderItem) {
      nonDiscountedDraftOrderItem.quantity = totalShopifyCartQuantity - (discountedDraftOrderItem?.quantity || 0);
    } else {
  ...
  if (totalDraftOrderQuantity > totalShopifyCartQuantity) {
    ...
    nonDiscountedQuantityToRemove = nonDiscountedDraftOrderItem
      ? Math.max(Math.min(nonDiscountedDraftOrderItem.quantity, quantityToRemove), 0)
      : 0;
    discountedQuantityToRemove = discountedDraftOrderItem
      ? Math.max(Math.min(discountedDraftOrderItem.quantity, quantityToRemove - nonDiscountedQuantityToRemove), 0)
      : 0;
    ...
    if (discountedDraftOrderItem) {
      ...
      await updateOfferHits(draftOrder, discountedDraftOrderItem, discountedQuantityToRemove);
    }
  }
};

const updateShopifyDraftOrderItems = async (shop, draftOrderId, shopifyCartItems) => {
  ...
  draftOrder.line_items = draftOrder.line_items.filter((lineItem) => lineItem.quantity > 0);

  // Find discounted line items not in the Shopify cart.
  discountedLineItemsNotInShopifyCart = draftOrder.line_items.filter((lineItem) => {
    const hasDiscount = !!lineItem.applied_discount;
    const variantIsInCart = !shopifyCartItems.find((item) => item.shopifyVariantId === lineItem.variant_id);
  ...
  draftOrder.line_items = draftOrder.line_items.filter(
    (lineItem) => !!shopifyCartItems.find((item) => item.shopifyVariantId === lineItem.variant_id)
  );

  // Update offer hits to stop tracking removed discounted items.
  await Promise.mapSeries(discountedLineItemsNotInShopifyCart, async (discountedLineItem) => {
    await updateOfferHits(draftOrder, discountedLineItem, discountedLineItem.quantity);
  });

  // Count the remaining discounted line items.
  discountedLineItemCount = draftOrder.line_items.filter((lineItem) => !!lineItem.applied_discount).length;
