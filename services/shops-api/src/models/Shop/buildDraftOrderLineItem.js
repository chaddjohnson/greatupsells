...
const buildDraftOrderLineItem = async ({ offerId, shopifyVariantId, quantity }) => {
  const [Product, Offer] = await Promise.all([models.get('Product'), models.get('Offer')]);
  ...
  if (offer && offer.discountType === 'PERCENTAGE') {
    lineItem.applied_discount = {
      ...
      amount: Math.round((price * quantity * offer.discountValue + Number.EPSILON) * 100) / 100
    };
    ...
    lineItem.applied_discount = {
      ...
      amount: Math.round((quantity * offer.discountValue + Number.EPSILON) * 100) / 100
    };
    ...
    lineItem.applied_discount = {
      ...
      value_type: 'fixed_amount',
      amount: Math.round(((price - offer.discountValue) * quantity + Number.EPSILON) * 100) / 100
    };
  }
