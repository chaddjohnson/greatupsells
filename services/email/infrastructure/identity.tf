...
resource "aws_route53_record" "domain_mx" {
  ...
  name    = "mail.${data.terraform_remote_state.greatupsells_infrastructure.outputs.domain}"
  type    = "MX"
  ttl     = "300"
...
resource "aws_route53_record" "domain_dkim_record" {
  count   = 3
  zone_id = data.terraform_remote_state.greatupsells_infrastructure.outputs.hosted_zone_id
  name    = "${element(aws_ses_domain_dkim.domain.dkim_tokens, count.index)}._domainkey.${data.terraform_remote_state.greatupsells_infrastructure.outputs.domain}"
  type    = "CNAME"
  ttl     = "300"
