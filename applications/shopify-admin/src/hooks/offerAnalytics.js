...
const useOfferAnalytics = (offerId, startAt, endAt) => {
  ...
  const { offerAcceptances, offerAcceptancesLoading, offerAcceptancesError, fetchOfferAcceptances } = useOfferAcceptances(
    offerId,
    startAt,
    endAt
  );
  const { offerConversions, offerConversionsLoading, offerConversionsError, fetchOfferConversions } = useOfferConversions(
    offerId,
    startAt,
    endAt
  );
  const { offerConversionRates, offerConversionRatesLoading, offerConversionRatesError, fetchOfferConversionRates } =
    useOfferConversionRates(offerId, startAt, endAt);
  const { offerRevenueIncreases, offerRevenueIncreasesLoading, offerRevenueIncreasesError, fetchOfferRevenueIncreases } =
    useOfferRevenueIncreases(offerId, startAt, endAt);
  const { offerImpressions, offerImpressionsLoading, offerImpressionsError, fetchOfferImpressions } = useOfferImpressions(
    offerId,
    startAt,
    endAt
  );
